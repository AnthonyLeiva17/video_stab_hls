...
void read_txt_and_prepare_input(const char* filename, ap_uint<32>* imgInput) {
    ...
    for (int i = 0; i < HEIGHT * WIDTH; i++) {
        infile >> b >> g >> r >> x; // Leer valores BGR
        ap_uint<32> pixel_pack = 0;
...
void print_imgInput(ap_uint<32>* imgInput) {
    for (int i = 0; i < HEIGHT * WIDTH; i++) {
...
int main() {
    ...
    ap_uint<32 > imgInput[HEIGHT * WIDTH]; // Buffer de entrada con 24 bits por píxel
    ap_uint<8 > imgOutput[HEIGHT * WIDTH]; // Buffer de salida con 8 bits por píxel
